...
class AvaPairs(data.Dataset):
    def __init__(self, phase="train", nb_positives=None, nb_videos=10, seed=0):
        self.w = 224
        self.h = 224
        self.alpha = 0.1
    ...
    def choose_videos(self):
        ...
        for file in pairs_files:
            video_id = "_".join(file.split("/")[-1].split(".")[0].split("_")[1:])
            all_video_ids.append(video_id)
        self.video_ids = random.sample(all_video_ids, self.nb_videos)

    def gather_positive_pairs(self):
        print("Gathering positive pairs")
        self.positive_pairs = []
        pairs_files = ["{}/{}/positive/pairs_{}.csv".format(self.pairs_dir, self.phase, video_id) for video_id in self.video_ids]
    ...
    def gather_negative_pairs(self):
        ...
        pairs_files = ["{}/{}/hard_negative/pairs_{}.csv".format(self.pairs_dir, self.phase, video_id) for video_id in self.video_ids]
        for file in tqdm.tqdm(pairs_files):
            with open(file, "r") as f:
                for line in f:
                    pair = line.strip().split(",")
        ...
        pairs_files = ["{}/{}/medium_negative/pairs_{}.csv".format(self.pairs_dir, self.phase, video_id) for video_id in self.video_ids]
        for file in tqdm.tqdm(pairs_files):
            with open(file, "r") as f:
                for line in f:
                    pair = line.strip().split(",")
